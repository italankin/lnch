...
public class LauncherDescriptorRepository implements DescriptorRepository {
    ...
    public LauncherDescriptorRepository(Context context, PackageManager packageManager,
        ...
        this.appDescriptors = new AppDescriptors(preferences, packageManager);
        this.loadFromFileInteractor = new LoadFromFileInteractor(packagesStore, descriptorStore,
                shortcutsRepository, packageManager, appDescriptors);
    ...
    private Single<AppsData> loadAll() {
        return Single
                ...
                .flatMap(infoList -> {
                    ...
                    return loadFromFileInteractor.loadFromFile(infoList)
        ...
        return Single
                .fromCallable(() -> {
                    ...
                    for (LauncherActivityInfo info : infoList) {
                        items.add(appDescriptors.create(info));
                    }
    ...
    static final class Editor implements DescriptorRepository.Editor {

        private final LauncherDescriptorRepository repository;
        private final Queue<DescriptorRepository.Editor.Action> actions = new ArrayDeque<>();
        private final List<Descriptor> items;
        private volatile boolean used;

        Editor(LauncherDescriptorRepository repository) {
        ...
        public Completable commit() {
            ...
            return Single
                    ...
                    .doOnSubscribe(onSubscribe)
                    .doOnSuccess(repository::writeToDisk)
                    .flatMapCompletable(descriptors -> repository.update());
        }
    }
}
